...
import { ArrowLeftIcon, ClockIcon, ArrowRightIcon, CheckIcon } from '../components/Icons.jsx'
...
const STORAGE_KEY = 'proofpath_completed'
...
export { loadCompleted, saveCompleted }   // used by Completion.jsx to auto-mark

export default function ProblemLibrary({ navigate }) {
  const [contest,    setContest]    = useState('All')
  const [topic,      setTopic]      = useState('All')
  const [difficulty, setDifficulty] = useState('All')
  const [showDone,   setShowDone]   = useState('all')   // 'all' | 'todo' | 'done'
  ...
  const filtered = PROBLEMS.filter(p =>
    ...
    (showDone   === 'all'  ||
     (showDone  === 'done' &&  completed.has(p.id)) ||
     (showDone  === 'todo' && !completed.has(p.id)))
  )

  const doneCount = PROBLEMS.filter(p => completed.has(p.id)).length
    ...
    <div className="page">
      ...
      <div className="flex items-center gap-12 mb-32">
        ...
        <div style={{
          ...
          background: doneCount === PROBLEMS.length ? 'var(--green-lt)' : 'var(--bg)',
          border: '1px solid var(--border)', borderRadius: 20,
          padding: '6px 14px', fontSize: '.82rem', fontWeight: 600,
          color: doneCount === PROBLEMS.length ? 'var(--green)' : 'var(--muted)',
          flexShrink: 0,
        }}>
          {doneCount === PROBLEMS.length && <CheckIcon size={13} />}
          {doneCount} / {PROBLEMS.length} done
        </div>
      </div>
      ...
      <div className="filters">
        ...
        <div style={{ display: 'flex', gap: 4, marginLeft: 'auto' }}>
          {[['all','All'], ['todo','To do'], ['done','Done']].map(([val, label]) => (
            <button
              key={val}
              onClick={() => setShowDone(val)}
              style={{
                fontSize: '.8rem', fontWeight: 600, padding: '6px 13px',
                borderRadius: 20, border: '1.5px solid',
                borderColor: showDone === val ? 'var(--primary)' : 'var(--border)',
                background:  showDone === val ? 'var(--primary-lt)' : 'var(--surface)',
                color:       showDone === val ? 'var(--primary)' : 'var(--muted)',
          ...
          {filtered.map(p => {
            ...
            return (
              <div
                ...
                style={{ opacity: isDone ? 0.72 : 1, position: 'relative' }}
              >
                {/* Checkbox — top right */}
                <button
                  title={isDone ? 'Mark as not done' : 'Mark as done'}
                  onClick={e => toggleDone(e, p.id)}
                  style={{
                ...
                {/* Meta badges */}
                <div className="prob-card__meta" style={{ paddingRight: 28 }}>
                  ...
                  {isDone && <span className="badge badge--verified">Done</span>}
                </div>

                {/* Title */}
                <p
                  className="prob-card__title"
                  style={{ textDecoration: isDone ? 'line-through' : 'none', color: isDone ? 'var(--muted)' : 'var(--text)' }}
                >
                  {p.title}
                </p>
